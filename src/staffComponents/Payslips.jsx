    ...
    <div className="p-6 grid gap-6">
      ...
      >
        <div className="flex flex-col md:flex-row justify-between items-center gap-3 mb-4">
          ...
          />
          <div className={`${isDarkMode ? "text-slate-400" : "text-blue-400"} text-sm`}>
            Showing {Math.min(filtered.length, currentPage * rowsPerPage) || 0} of {filtered.length} results
          </div>
        </div>

        {/* Table */}
        <div className="overflow-x-auto">
          <table className={`w-full table-auto text-center transition ${isDarkMode ? "text-slate-200" : "text-gray-800"}`}>
            <thead>
                ...
                {["Month", "Gross", "Deductions", "Net", "Action"].map((h) => (
                  <th
                    ...
                    className={`p-3 uppercase tracking-wider transition ${
                      isDarkMode ? "text-teal-400 hover:text-white" : "text-gray-700 hover:text-gray-900"
                    }`}
                  >
                    {h}
                  </th>
              ...
              {paginated.length > 0 ? (
                paginated.map((r) => (
                  ...
                  >
                    ...
                    <td className={`p-3 font-semibold ${isDarkMode ? "" : "text-blue-600"}`}>{formatAmount(r.netSalary)}</td>
                    <td className="p-3">
                      <button
                        onClick={() => downloadPayslip(r)}
                        className={`px-3 py-1 rounded ${
                          isDarkMode ? "bg-teal-600 text-white hover:bg-teal-700" : "bg-blue-400 text-white hover:bg-blue-500"
                        }`}
                ...
                <tr>
                  <td
                    className={`p-3 italic ${isDarkMode ? "text-slate-400" : "text-gray-500"}`}
                    colSpan={5}
                  >
                    No results
                  </td>
        ...
        <div className="flex items-center justify-between mt-4">
          ...
          <div className="flex gap-2">
            <button
              ...
              className={`px-3 py-1 rounded ${
                isDarkMode ? "bg-slate-700 text-slate-200" : "bg-blue-100 text-blue-400"
              } disabled:opacity-50`}
            >
              Prev
            </button>

            {/* simple prev/next; add numeric pages if needed */}
            <button
