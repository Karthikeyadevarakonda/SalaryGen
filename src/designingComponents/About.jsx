...
const colors = {
  light: {
    ...
    iconBg: ["from-blue-400 to-cyan-400", "from-pink-500 to-rose-400", "from-teal-400 to-emerald-400", "from-indigo-500 to-purple-500", "from-yellow-400 to-yellow-300"],
  ...
  dark: {
    ...
    cardBg: "bg-[#2c313a]",
    cardText: "text-white",
    iconBg: ["from-cyan-400 to-teal-400", "from-rose-400 to-pink-500", "from-emerald-400 to-teal-400", "from-purple-500 to-indigo-500", "from-yellow-400 to-yellow-500"],
    buttonBg: "bg-transparent",
...
const TeamCard = ({ member, idx, theme }) => {
  ...
  return (
    ...
    >
      <div className={`w-16 h-16 rounded-full flex items-center justify-center mb-4 bg-gradient-to-r ${t.iconBg[idx % t.iconBg.length]} text-white shadow-md`}>
        <Icon className="w-7 h-7" />
      </div>
      <h3 className={`text-lg sm:text-xl font-semibold mb-1 text-center ${t.cardText}`}>{member.name}</h3>
      <p className={`text-sm text-center ${t.textSecondary}`}>{member.role}</p>
      <p className={`text-xs text-center mt-3 px-2 ${t.textMuted}`}>{member.bio}</p>
    </motion.div>
  );
};
...
const CarouselItem = ({ member, idx, onOpen, theme }) => {
  ...
  const t = colors[theme];
  return (
    <motion.div whileTap={{ scale: 0.97 }} whileHover={{ scale: 1.02 }} className="snap-center flex-shrink-0 w-64 sm:w-72 p-4 rounded-xl">
      <div className={`rounded-xl p-4 h-full flex flex-col items-center justify-between shadow-lg ${t.cardBg}`}>
        <div className="flex flex-col items-center gap-3">
          <div className={`w-20 h-20 rounded-full flex items-center justify-center bg-gradient-to-r ${t.iconBg[idx % t.iconBg.length]} text-white text-xl shadow-inner`}>
            <Icon className="w-8 h-8" />
          </div>
          <div className="text-center">
            <h4 className={`text-lg font-semibold leading-tight ${t.cardText}`}>{member.name}</h4>
...
const About = () => {
  const { isDarkMode } = useTheme(); // ✅ useTheme hook
  ...
  return (
    <div className={`${t.pageBg} min-h-screen py-12 px-4 sm:px-6 lg:px-12`}>
      <header className="max-w-4xl mx-auto text-center mb-8">
        ...
        <p className={`mt-3 max-w-xl mx-auto text-sm sm:text-base ${t.textMuted}`}>
      ...
      <section className="md:hidden mt-4">
        <div className="relative">
          <div ref={carouselRef} className="flex gap-4 overflow-x-auto px-4 pb-4 scroll-smooth snap-x snap-mandatory" style={{ WebkitOverflowScrolling: "touch" }}>
            {team.map((m, i) => (
              <CarouselItem key={m.name} member={m} idx={i} onOpen={openModal} theme={theme} />
            ))}
          </div>
      ...
      <AnimatePresence>
        {modal.open && modal.member && (
          <motion.div className="fixed inset-0 z-50 flex items-center justify-center" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            <div className={`${t.overlay}`} onClick={() => setModal({ open: false, member: null, idx: 0 })} />
            <motion.div
            ...
            >
              ...
              <div className="flex flex-col items-center gap-3">
                <div className={`w-24 h-24 rounded-full flex items-center justify-center bg-gradient-to-r ${t.iconBg[modal.idx % t.iconBg.length]} text-white shadow-md`}>
                  <modal.member.icon className="w-10 h-10" />
                </div>
                <h3 className={`text-xl font-bold ${t.modalText}`}>{modal.member.name}</h3>
                <p className={`text-sm ${t.textSecondary}`}>{modal.member.role}</p>
                <p className={`text-center mt-2 ${t.textMuted}`}>{modal.member.bio}</p>
              </div>
            </motion.div>
          </motion.div>
        )}
