 ...
 const DEPARTMENTS = ["CSE", "HR", "EEE", "ECE", "MECH","CIVIL"];
  ...
  const ui = {
    ...
    title: isDarkMode
      ...
      : "text-blue-600",
    cardWrap: `rounded shadow border ${isDarkMode ? "border-slate-700 bg-slate-900/70" : "border-slate-200 bg-white"} backdrop-blur-lg`,
    thead: `uppercase sticky top-0 z-10 ${isDarkMode ? "bg-slate-800/50 text-teal-300" : "bg-slate-100 text-blue-700"}`,
    row: isDarkMode ? "border-b border-slate-700 hover:bg-slate-800/60" : "border-b border-slate-200 hover:bg-slate-50",
    input: `w-full p-3 rounded-xl ${isDarkMode ? "bg-slate-800/70 border-slate-700" : "bg-white border-slate-300"} border backdrop-blur-sm focus:outline-none ${isDarkMode ? "focus:border-teal-400" : "focus:border-blue-500"}`,
  ...
  useEffect(() => { setTimeout(()=>fetchData(),1000) }, [baseUrl]);
...
const payload = {
  ...
  return (
    <div className={`px-2 h-full ${ui.pageBg}`}>
      <h1 className={`text-lg sm:text-xl font-semibold mb-2 ${isDarkMode ? "text-teal-400" : "text-blue-400"}`}>
        Salary Components
      </h1>
...
/>
  ...
  {loading ? (
    ...
    >
      ...
      {/* Form Sidebar */}
      <div className={`fixed top-0 right-0 h-full w-full max-w-lg ${ui.sidebar} shadow-2xl border-l z-50 transform transition-transform duration-500 ease-in-out ${isFormOpen ? "translate-x-0" : "translate-x-full"}`}>
...
<div
  className={`fixed top-0 right-0 h-full w-full max-w-md ${ui.sidebar} shadow-2xl border-l z-50 transform transition-transform duration-500 ease-in-out ${
